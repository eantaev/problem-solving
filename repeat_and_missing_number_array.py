...
def solve(vs):
    ...
    vs_xor = xor(vs)
    full_xor = xor(full)
    terminator = least_significant_one(vs_xor ^ full_xor)

    v0_xor, v1_xor = split_xor(vs, terminator)
    f0_xor, f1_xor = split_xor(full, terminator)
    c = v0_xor ^ f0_xor
    d = v1_xor ^ f1_xor

    return choose_duplicate_and_missing(vs, c, d)


def choose_duplicate_and_missing(vs, candidate1, candidate2):
    for v in vs:
        if v == candidate1:
            return [candidate1, candidate2]
        elif v == candidate2:
            return [candidate2, candidate1]
    raise ValueError('Neither of candidates is in vs')
