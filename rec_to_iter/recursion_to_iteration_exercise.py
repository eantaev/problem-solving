...
def find_val_or_next_smallest_tail(bst, x, closest_so_far=None):
    """Get the greatest value <= x in a binary search tree.
    Returns None if no such value can be found.
    """
    if bst is None:
        ...
        return find_val_or_next_smallest_tail(bst.left, x, closest_so_far)
    else:
        return find_val_or_next_smallest_tail(bst.right, x, closest_so_far=bst.val)


def find_val_or_next_smallest_tail1(bst, x, closest_so_far=None):
    """Get the greatest value <= x in a binary search tree.
    Returns None if no such value can be found.
    """
    while True:
        if bst is None:
            ...
            return find_val_or_next_smallest_tail1(bst.left, x, closest_so_far)
        else:
            return find_val_or_next_smallest_tail1(bst.right, x, closest_so_far=bst.val)
        break


def find_val_or_next_smallest_iterative1(bst, x, closest_so_far=None):
    """Get the greatest value <= x in a binary search tree.
    Returns None if no such value can be found.
    """
    while True:
        if bst is None:
            ...
            (bst, x, closest_so_far) = (bst.left, x, closest_so_far)
            continue
        else:
            (bst, x, closest_so_far) = (bst.right, x, bst.val)
            continue
        break
...
def find_val_or_next_smallest_iterative2(bst, x, closest_so_far=None):
    """Get the greatest value <= x in a binary search tree.
    Returns None if no such value can be found.
    """
    while True:
        if bst is None:
            ...
            (bst, closest_so_far) = (bst.right, bst.val)
...
class TestFindValOrNextSmallest(unittest.TestCase):
    def test_recursive(self):
        test_fn(find_val_or_next_smallest)

    def test_tailrec(self):
        test_fn(find_val_or_next_smallest_tail)

    def test_tailrec1(self):
        test_fn(find_val_or_next_smallest_tail1)

    def test_iterative1(self):
        test_fn(find_val_or_next_smallest_iterative1)
